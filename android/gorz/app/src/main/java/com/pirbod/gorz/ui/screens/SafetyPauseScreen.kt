...
fun SafetyPauseScreen(
    ...
    var reason by remember(state.safetyState.reason) { mutableStateOf(state.safetyState.reason) }
    LazyColumn(
        ...
        item {
            Text("Current safety state: ${state.safetyState.source} · ${if (state.safetyState.paused) "paused" else "ready"}")
        }
    }
}
